...
if __name__ == "__main__":
    ...
    parser.add_argument("--image-extension", default=".jpg", type=str, help="Extension of files in image_path")
    parser.add_argument("--database-path", default="database.db", help="Location where the COLMAP .db file will be created")
    parser.add_argument(
        "--single-camera",
        action="store_true",
        help=("Consider all photos to be made with a single camera (COLMAP " "will reduce the number of degrees of freedom"),
    )
    parser.add_argument(
        "--camera-model",
        choices=["simple-pinhole", "pinhole", "simple-radial", "opencv"],
        default="simple-radial",
        help=("Camera model to use in COLMAP. " "See https://github.com/colmap/colmap/blob/master/src/base/camera_models.h" " for explanations"),
    )
    ...
    if args.camera_model == "opencv" and not args.single_camera:
        raise RuntimeError(
            "Cannot use --camera-model=opencv camera without " "--single-camera (the COLMAP optimisation will " "likely fail to converge)"
        )
    ...
    fname_to_id = add_keypoints(db, args.h5_path, args.image_path, args.image_extension, args.camera_model, args.single_camera)
    add_matches(
        db,
        args.h5_path,
        fname_to_id,
